 ...
 * the intention that was active when it ran, and how it was closed. A `null` [outcome] means the
 * session was aborted via the Stop button, which carries no closure choice; a non-null value is
 * the explicit dialog outcome. Engaged / deep-focus durations are NOT stored — they are derived at
 * render time from the day-wide interval lists.
 */
 ...
 * (epoch millis; outcome by enum name, or empty when the session was aborted; intention Base64 so
 * its commas / newlines survive as a safe last field). Empty list encodes to just the marker.
 */
...
/**
 * Inverse of [encodeFocusSessionRecords]; blank / malformed lines are skipped. An empty outcome
 * field decodes to a valid record with `outcome = null` (aborted); a non-empty but unknown outcome
 * name still drops the line (forward-compat with a newer peer).
 */
@OptIn(ExperimentalEncodingApi::class)
fun decodeFocusSessionRecords(encoded: String): List<FocusSessionRecord> {
    ...
    return bodyLines.mapNotNull { line ->
        ...
        val outcomeName = parts[2]
        val outcome = if (outcomeName.isEmpty()) {
            null
        } else {
            FocusClosureOutcome.entries.firstOrNull { it.name == outcomeName } ?: return@mapNotNull null
        }
        val intention = runCatching { Base64.decode(parts[3]).decodeToString() }.getOrNull() ?: return@mapNotNull null
