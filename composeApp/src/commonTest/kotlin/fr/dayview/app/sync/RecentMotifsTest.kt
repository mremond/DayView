...
class RecentMotifsTest {
    ...
    fun buildBoundsTombstonesWithRecurringLiveMotif() {
        ...
        // it keeps round 0's ancient stamp forever. The other MAX_RECENT_DETOUR_MOTIFS - 1 slots
        // churn: each round introduces a fresh disjoint set, which turns into tombstones the next
        // round. Live count per round is exactly MAX_RECENT_DETOUR_MOTIFS (1 pinned + churn), so
        ...
        val churnPerRound = MAX_RECENT_DETOUR_MOTIFS - 1
        repeat(MAX_RECENT_DETOUR_MOTIFS + 5) { round ->
